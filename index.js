...
function buildDebugReport() {
  const lines = [`[simple extension] v0.7.50 debug report`];
...
function initialize() {
  ...
  console.info("[simple extension] v0.7.50 loaded — native SillyTavern layout themed");
  return true;
}
...
globalThis.simpleExtensionDebug = (filter = "") => {
  ...
  const lines = [`[simple extension] v0.7.50 debug dump`];
  state.nativeUnits.forEach((unit) => {
